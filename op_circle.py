...
class OBJECT_OT_select_circle_xray(bpy.types.Operator):
    ...
    def select_circle_xray(self, context):
        ...
        bpy.ops.view3d.select_circle('INVOKE_DEFAULT', mode=self.mode, wait_for_input=self.wait_for_input, radius=self.radius)
    ...
    def finish(self, context):
        ...
        # store view3d.select_circle radius in the current op properties to use it in the next invoke
...
class MESH_OT_select_circle_xray(bpy.types.Operator):
    ...
    def invoke(self, context, event):
        # if select through is disabled, use default "circle select" without any optional features
        ...
        context.window_manager.modal_handler_add(self)
        # start default circle select modal
        bpy.ops.view3d.select_circle('INVOKE_DEFAULT', mode=self.mode, wait_for_input=self.wait_for_input, radius=self.radius)
        return {'RUNNING_MODAL'}
